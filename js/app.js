...
function creatorNavAuditLedgerAppend(type,payload={}){
  ...
  let raw=JSON.stringify(body),h=0;for(let i=0;i<raw.length;i++)h=((h<<5)-h+raw.charCodeAt(i))|0;
  body.signature="sig-"+Math.abs(h).toString(16);ledger.push(body);creatorNavAuditLedgerWrite(ledger);return body;
}

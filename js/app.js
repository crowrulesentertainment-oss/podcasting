...
function creatorNavAssign(id,key,memberId){
  const lock=creatorNavPresenceState.taskLocks?.[id+"::"+key],me=creatorNavPresenceIdentity();
  if(lock&&lock.actorId!==me.id){alert("Task is locked by "+String(lock.ownerName||lock.actorName||"another creator")+" for collaborative editing.");return false;}
...
function creatorNavNotificationBadge(){
  ...
  document.querySelectorAll("[data-creator-notification-badge]").forEach(b=>{b.textContent=u+(a?" · "+a+" action":"");b.hidden=!u;});
}
...
function renderCollaborativeNotificationsPanel(){
  ...
  const target=document.getElementById("creatorNavCollaborativeControlRoom")||document.body;
  const box=document.createElement("div");box.id=id;box.className="card";box.style.cssText="padding:8px;margin-top:10px";target.parentNode?.insertBefore(box,target);
  renderCollaborativeNotifications();
}
...
;
/* 9.2 PRODUCTION STATE CUTOVER */
const CREATOR_NAV_CUTOVER_KEY="crowrules_creator_server_cutover_v1",CREATOR_NAV_BACKUP_KEY="crowrules_creator_server_backup_9_2_v1";
function creatorNavCutoverRead(){try{return JSON.parse(localStorage.getItem(CREATOR_NAV_CUTOVER_KEY)||"null");}catch{return null;}}
function creatorNavCutoverActive(){return !!creatorNavCutoverRead()?.committed;}
function creatorNavCutoverWrite(x){try{localStorage.setItem(CREATOR_NAV_CUTOVER_KEY,JSON.stringify(x));}catch{}}
function creatorNavProductionWriteBlocked(){return creatorNavCutoverActive();}
function creatorNavServerState(){return window.creatorNavServerProductionState||{roadmaps:[],steps:[],tasks:[],health:[],capacity:[],metrics:{}};}
async function creatorNavServerMigrationStatus(){try{return await creatorNavServerCall("migration-status",{});}catch(e){return {migrations:[],error:String(e?.message||e)};}}
async function creatorNavServerMigrationPreview(state,sourceHash){return creatorNavServerCall("migration-preview",{source_version:"9.2",source_hash:sourceHash,state});}
async function creatorNavServerMigrationCommit(migrationId,state,sourceHash,allowConflicts=false){return creatorNavServerCall("migration-commit",{migration_id:migrationId,source_hash:sourceHash,state,allow_conflicts:!!allowConflicts});}
async function creatorNavServerProductionState(){try{const r=await creatorNavServerCall("production-state",{});window.creatorNavServerProductionState=r;return r;}catch(e){console.warn("Server production state unavailable",e);return null;}}
async function creatorNavServerTaskClaim(id,key,expectedVersion=null){return creatorNavServerCall("task-claim",{collection_id:id,task_key:key,expected_version:expectedVersion});}
async function creatorNavServerTaskComplete(id,key,expectedVersion=null){return creatorNavServerCall("task-complete",{collection_id:id,task_key:key,expected_version:expectedVersion});}
async function creatorNavServerTaskStatus(id,key,status,expectedVersion=null){return creatorNavServerCall("task-status",{collection_id:id,task_key:key,status,expected_version:expectedVersion});}
function creatorNavMigrationUUID(v,currentUserId){const s=String(v||"");if(/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(s))return s;if(["me","current","current-user"].includes(s))return currentUserId||null;return null;}
function creatorNavMigrationISO(v){if(!v)return null;const d=new Date(v);return Number.isNaN(d.getTime())?null:d.toISOString();}
function creatorNavMigrationState(rows,userId){const roadmaps=[],steps=[],tasks=[],history=[],ownerConflicts=[];(rows||[]).forEach(x=>{const id=String(x?.c?.id||"");if(!id)return;const target=creatorNavPlanTarget(id),targetDate=target?.targetDate?creatorNavMigrationISO(target.targetDate+"T23:59:59"):null,ex=(creatorNavExecutionRead()[id]||{tasks:{},history:[],assignments:{}}),assignments=ex.assignments||{},rs=creatorNavRoadmapSteps(x,x.target)||[];roadmaps.push({id,name:String(x?.c?.name||id),target_date:targetDate,velocity:Number(x?.f?.velocity||0)});rs.forEach((s,i)=>{const owner=creatorNavMigrationUUID(assignments[s.key],userId);if(assignments[s.key]&&!owner)ownerConflicts.push({type:"OWNER_MAPPING",collection_id:id,task_key:s.key,owner:String(assignments[s.key])});const d=target?.targetDate?new Date(target.targetDate+"T23:59:59").getTime()-Math.max(0,rs.length-1-i)*86400000:null,iso=d?new Date(d).toISOString():targetDate,done=!!s.done;steps.push({roadmap_id:id,step_key:String(s.key),step_name:String(s.name||s.key),ordinal:i,target_date:iso,dependency_key:i?String(rs[i-1].key):null,owner_user_id:owner,status:done?"COMPLETE":((ex.tasks||{})[s.key]?"IN_PROGRESS":"OPEN")});tasks.push({collection_id:id,task_key:String(s.key),task_name:String(s.name||s.key),owner_user_id:owner,status:done?"COMPLETE":((ex.tasks||{})[s.key]?"IN_PROGRESS":"OPEN"),target_date:iso,started_at:null,completed_at:done?creatorNavMigrationISO((ex.history||[]).find(h=>h.key===s.key&&h.done)?.at):null,last_activity_at:creatorNavMigrationISO((ex.history||[]).filter(h=>h.key===s.key).slice(-1)[0]?.at),dependency_key:i?String(rs[i-1].key):null});});(ex.history||[]).forEach(h=>history.push({roadmap_id:id,step_key:String(h.key||""),user_id:creatorNavMigrationUUID(h.user_id||h.userId,userId),action:h.done?"COMPLETE":"REOPEN",from_status:h.done?"IN_PROGRESS":"COMPLETE",to_status:h.done?"COMPLETE":"OPEN",occurred_at:creatorNavMigrationISO(h.at)||new Date().toISOString(),metadata:{name:h.name||h.key,source:"localStorage",source_version:"9.2"}}));});return{roadmaps,steps,tasks,history,ownerConflicts,generated_at:new Date().toISOString(),schema_version:"9.2"};}
async function creatorNavMigrationSourceHash(state){const h=await crypto.subtle.digest("SHA-256",new TextEncoder().encode(JSON.stringify(state)));return Array.from(new Uint8Array(h)).map(b=>b.toString(16).padStart(2,"0")).join("");}
function creatorNavMigrationBackup(){try{localStorage.setItem(CREATOR_NAV_BACKUP_KEY,JSON.stringify({createdAt:new Date().toISOString(),roadmaps:creatorNavPlanningRead(),execution:creatorNavExecutionRead(),team:creatorNavTeamRead(),board:typeof creatorNavBoardRead==="function"?creatorNavBoardRead():null}));}catch{}}
function renderProductionCutoverPanel(){let host=document.getElementById("creatorNavProductionCutover"),target=document.getElementById("creatorNavLiveControlRoom");if(!host&&target){host=document.createElement("div");host.id="creatorNavProductionCutover";target.parentNode?.insertBefore(host,target);}if(!host)return;const cut=creatorNavCutoverRead();host.innerHTML=cut?.committed?'<div class="card" style="padding:10px;margin-bottom:8px;border:1px solid rgba(80,255,180,.35)"><b>9.2 PRODUCTION CUTOVER · ACTIVE</b><div style="font-size:.72rem;margin-top:4px">Supabase is the only writable production source. Browser localStorage is read-only fallback.</div><div style="font-size:.68rem;margin-top:4px">Migration '+escText(cut.migrationId||"—")+' · '+escText(cut.committedAt||"—")+'</div></div>':'<div class="card" style="padding:10px;margin-bottom:8px"><b>9.2 PRODUCTION STATE CUTOVER</b><div style="font-size:.72rem;margin-top:4px">Preview and verify the one-time migration before making Supabase authoritative.</div><button id="creatorNavMigrationPreview" style="margin-top:7px">PREVIEW MIGRATION</button></div>';document.getElementById("creatorNavMigrationPreview")?.addEventListener("click",creatorNavRunMigrationPreview);}
async function creatorNavRunMigrationPreview(){const host=document.getElementById("creatorNavProductionCutover");if(!host)return;try{const ctx=await creatorNavIdentityContext();if(!ctx.userId||!ctx.serverAuthorized)throw new Error("Active Creator Governance authorization required.");const state=creatorNavMigrationState(data,ctx.userId),hash=await creatorNavMigrationSourceHash(state),r=await creatorNavServerMigrationPreview(state,hash);if(r.already_cutover&&r.migration){creatorNavCutoverWrite({committed:true,migrationId:r.migration.id,sourceHash:r.migration.source_hash,committedAt:r.migration.committed_at,verification:r.migration.verification});renderProductionCutoverPanel();return;}const p=r.preview||{},c=p.counts||{},conflicts=p.conflicts||[];host.innerHTML='<div class="card" style="padding:10px;margin-bottom:8px"><b>9.2 MIGRATION PREVIEW</b><div style="font-size:.72rem;margin-top:4px">Roadmaps '+(c.roadmaps||0)+' · Steps '+(c.steps||0)+' · Tasks '+(c.tasks||0)+' · History '+(c.history||0)+'</div><div style="font-size:.72rem;margin-top:4px">Existing server: '+(c.existing_roadmaps||0)+' roadmaps · '+(c.existing_steps||0)+' steps · '+(c.existing_tasks||0)+' tasks</div><div style="font-size:.72rem;margin-top:4px">Owner conflicts: <b>'+conflicts.length+'</b></div>'+conflicts.slice(0,8).map(x=>'<div style="font-size:.67rem;margin-top:3px">⚠ '+escText(x.collection_id||"")+" / "+escText(x.task_key||"")+" · "+escText(x.owner||"unknown")+'</div>').join("")+'<div style="margin-top:7px"><button id="creatorNavMigrationCommit">COMMIT CUTOVER</button>'+(conflicts.length?'<button id="creatorNavMigrationCommitConflicts">IMPORT UNASSIGNED CONFLICTS</button>':"")+'</div></div>';const commit=async(allow)=>{try{creatorNavMigrationBackup();const out=await creatorNavServerMigrationCommit(r.migration.id,state,hash,allow);if(!out.cutover)throw new Error(out.error||"Migration not committed.");creatorNavCutoverWrite({committed:true,migrationId:r.migration.id,sourceHash:hash,committedAt:new Date().toISOString(),verification:out.verification});await creatorNavServerProductionRefresh();renderProductionCutoverPanel();renderProductionControlRoom();}catch(e){alert("9.2 migration failed: "+String(e?.message||e));}};document.getElementById("creatorNavMigrationCommit")?.addEventListener("click",()=>commit(false));document.getElementById("creatorNavMigrationCommitConflicts")?.addEventListener("click",()=>commit(true));}catch(e){host.innerHTML='<div class="card" style="padding:10px"><b>9.2 CUTOVER ERROR</b><div style="font-size:.72rem;margin-top:4px">'+escText(String(e?.message||e))+'</div></div>';}}
function creatorNavServerControlRoomState(){const s=creatorNavServerState(),tasks=s.tasks||[],health=s.health||[],capacity=s.capacity||[],open=tasks.filter(t=>t.status!=="COMPLETE"),risks=health.filter(h=>["BLOCKED","OVERDUE","STALLED","CRITICAL"].includes(h.health_status)),cap=capacity.reduce((n,x)=>n+Number(x.capacity_units||0),0),load=capacity.reduce((n,x)=>n+Number(x.assigned_units||0),0);return{tasks,health,capacity,open,risks,cap,load,utilization:cap?Math.round(load/cap*100):0,metrics:s.metrics||{}};}
function renderProductionControlRoom(){const box=document.getElementById("creatorNavLiveControlRoom");if(!box)return;if(creatorNavCutoverActive()){const q=creatorNavServerControlRoomState(),m=q.metrics||{};box.innerHTML='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:7px;margin-top:7px"><div class="card" style="padding:7px"><b>'+q.open.length+'</b><small> OPEN TASKS</small></div><div class="card" style="padding:7px"><b>'+q.risks.length+'</b><small> HEALTH RISKS</small></div><div class="card" style="padding:7px"><b>'+q.utilization+'%</b><small> SERVER UTILIZATION</small></div><div class="card" style="padding:7px"><b>'+Number(m.velocity_per_day||0).toFixed(2)+'</b><small> SERVER VELOCITY / DAY</small></div></div><div style="margin-top:8px"><b>SERVER CAPACITY</b>'+q.capacity.map(r=>'<div style="font-size:.72rem;margin-top:3px">'+escText(r.user_id)+' · '+Number(r.assigned_units||0)+' / '+Number(r.capacity_units||0)+' · '+Number(r.available_units||0)+' available</div>').join("")+'</div><div style="margin-top:8px"><b>SERVER HEALTH</b>'+(q.risks.slice(0,8).map(h=>'<div style="font-size:.72rem;margin-top:3px">⚠ '+escText(h.collection_id)+' / '+escText(h.task_key)+' · '+escText(h.health_status)+' · '+Number(h.health_score||0)+'/100</div>').join("")||'<div style="font-size:.72rem;opacity:.7">No active server health risks.</div>')+'</div><div style="margin-top:8px"><b>SERVER TASK STATE</b>'+(q.open.slice(0,12).map(t=>'<div style="font-size:.72rem;margin-top:3px">'+escText(t.task_name||t.task_key)+' · '+escText(t.status)+' · '+escText(t.owner_user_id||"Unassigned")+'</div>').join("")||'<div style="font-size:.72rem;opacity:.7">No open server tasks.</div>')+'</div>';return;}const q=creatorNavControlRoomState(data);box.innerHTML='<div style="font-size:.72rem">'+q.adaptive.collections.reduce((n,c)=>n+c.rows.length,0)+' open tasks · '+q.adaptive.conflicts.length+' conflicts</div>';}
const creatorNavLegacyExecutionWrite_92=creatorNavExecutionWrite,creatorNavLegacyPlanningWrite_92=creatorNavPlanningWrite,creatorNavLegacyTeamWrite_92=creatorNavTeamWrite,creatorNavLegacyBoardWrite_92=typeof creatorNavBoardWrite==="function"?creatorNavBoardWrite:null;
function creatorNavExecutionWrite(x){if(creatorNavProductionWriteBlocked())return false;return creatorNavLegacyExecutionWrite_92(x);}
function creatorNavPlanningWrite(x){if(creatorNavProductionWriteBlocked())return false;return creatorNavLegacyPlanningWrite_92(x);}
function creatorNavTeamWrite(x){if(creatorNavProductionWriteBlocked())return false;return creatorNavLegacyTeamWrite_92(x);}
function creatorNavBoardWrite(x){if(creatorNavProductionWriteBlocked())return false;return creatorNavLegacyBoardWrite_92?creatorNavLegacyBoardWrite_92(x):false;}
async function creatorNavExecutionToggle(id,key,done,name){const lock=creatorNavPresenceState.taskLocks?.[id+"::"+key],me=creatorNavPresenceIdentity();if(lock&&lock.actorId!==me.id){alert("Task is currently being worked by "+String(lock.ownerName||lock.actorName||"another creator")+".");return false;}try{const current=(creatorNavServerState().tasks||[]).find(t=>t.collection_id===id&&t.task_key===key);if(done){const claimed=await creatorNavPresenceWorking(id,key,name);if(!claimed?.ok)return false;const srv=await creatorNavServerTaskClaim(id,key,current?.version??null);if(!srv?.task)throw new Error("SERVER_CLAIM_FAILED");const complete=await creatorNavServerTaskComplete(id,key,srv.task.version);if(!complete?.task)throw new Error("SERVER_COMPLETION_FAILED");await creatorNavPresenceUnlock(id,key);}else{const reopened=await creatorNavServerTaskStatus(id,key,"OPEN",current?.version??null);if(!reopened?.task)throw new Error("SERVER_REOPEN_FAILED");await creatorNavPresenceUnlock(id,key);}await creatorNavServerProductionRefresh();return true;}catch(e){alert("Production state changed on another device or the server rejected the update: "+String(e?.message||e));await creatorNavServerProductionState();renderProductionControlRoom();return false;}}
async function creatorNavServerProductionRefresh(){await creatorNavServerProductionState();await creatorNavServerHealthRefresh();await creatorNavServerNotificationsRefresh();renderProductionCutoverPanel();renderProductionControlRoom();}
function creatorNav92Boot(){if(typeof supabase==="undefined")return;if(document.getElementById("creatorNavLiveControlRoom")){renderProductionCutoverPanel();creatorNavServerProductionRefresh();}creatorNavRealtimeStart();setTimeout(creatorNav92Boot,5000);}
setTimeout(creatorNav92Boot,500);

...
async function load(){
 ...
 const {data,error}=await supabase.from("podcasts").select("id,title,slug,description,artwork_url,status,category").eq("status","published").order("title");
 ...
 el.innerHTML=data.map(p=>'<article class="workspace-row"><div class="workspace-art">'+(p.artwork_url?'<img src="'+esc(p.artwork_url)+'" alt="">':'<span>CR</span>')+'</div><div class="workspace-main"><div class="eyebrow">'+esc(p.category||"PODCAST")+'</div><h2>'+esc(p.title)+'</h2><p>'+esc(p.description||"Live CrowRules podcast feed.")+'</p><div class="muted">RSS feed • Updates from published episodes</div></div><div class="workspace-actions"><a class="btn primary" target="_blank" rel="noopener" href="'+base+encodeURIComponent(p.slug)+'">Open RSS</a><button class="btn" data-copy="'+base+encodeURIComponent(p.slug)+'">Copy feed</button></div></article>').join("");

...
const waitForClient=async()=>{for(let i=0;i<60;i++){const c=getClient();if(c)return c;await new Promise(r=>setTimeout(r,100));}return null};
...
["members","podcast_member_profiles","podcast_member_profile_stats","membership_subscriptions","creator_profiles","creators","cr_podcast_stripe_accounts","cr_podcast_entitlements","podcast_notifications"].forEach(table=>channel.on("postgres_changes",{event:"*",schema:"public",table,filter:"user_id=eq."+uid},scheduleLoad));
...
async function init(){if(started)return state;started=true;db=await waitForClient();if(!db){emit();return null}const r=await db.auth.getUser();if(!r.data?.user){emit();return null}state.user=r.data.user;derive();emit();await load();startRealtime();authSub=db.auth.onAuthStateChange(async(ev,s)=>{stopRealtime();state.user=s?.user||null;if(!state.user){Object.assign(state,{member:null,profile:null,stats:null,membership:null,creator:null,creatorRecord:null,stripe:null,entitlement:null,premium:false,notifications:0});derive();state.status.realtime="offline";emit()}else{derive();emit();await load();startRealtime()}});timer=setInterval(()=>state.user&&load().catch(()=>{}),CFG.fallbackRefreshMs);return state}

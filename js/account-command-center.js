...
let db=null,state={user:null,profile:null,stats:null,membership:null,creator:null,stripe:null,notifications:0,status:{account:"signed_out",membership:"none",creator:"available",stripe:"not_connected",notifications:"clear",realtime:"offline"},updated_at:null};
...
]);const v=i=>q[i].status==="fulfilled"?q[i].value:null;state.profile=v(0)?.data||null;state.stats=v(1)?.data||null;state.membership=v(2)?.data||null;state.creator=v(3)?.data||null;state.stripe=v(4)?.data||null;state.notifications=v(5)?.count||0;derive();emit();return state}
...
["podcast_member_profiles","podcast_member_profile_stats","membership_subscriptions","creator_profiles","cr_podcast_stripe_accounts","podcast_notifications"].forEach(table=>channel.on("postgres_changes",{event:"*",schema:"public",table,filter:"user_id=eq."+uid},scheduleLoad));

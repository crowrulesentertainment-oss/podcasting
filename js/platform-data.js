...
async function signOut(){unsubscribeAll();if(!db)return;await db.auth.signOut()}
window.CrowRulesData={ready:boot(),createStateElement,renderState,bindStates,getClient:()=>db,getClientAsync,getAccessToken,getState:()=>structuredClone(state),getUser,requireAuth,query,subscribe,unsubscribeAll,guard,run,refresh,signOut,on:fn=>{listeners.add(fn);return()=>listeners.delete(fn)}};

...
function boot(){if(!window.supabase||!url||!key)return;const sb=window.crSupabase||window.CROWRULES_SUPABASE||window.supabase.createClient(url,key,{auth:{persistSession:true,autoRefreshToken:true}});window.crSupabase=sb;
...
async function sync(){const {data:{user}}=await sb.auth.getUser();const dot=strip.querySelector(".cr-monetization-dot"),text=strip.querySelector("span");if(!user){dot.className="cr-monetization-dot off";text.innerHTML='<strong>CROWRULES MONETIZATION</strong> · Sign in to access premium playback, library, and creator tools.';return}const r=await sb.rpc("get_my_podcast_playback_security_summary");if(r.error){dot.className="cr-monetization-dot warn";text.innerHTML='<strong>CROWRULES MONETIZATION</strong> · Account connected · Security status unavailable.';return}dot.className="cr-monetization-dot";text.innerHTML='<strong>CROWRULES MONETIZATION</strong> · Account connected · '+(r.data?.active_sessions??0)+' active playback session'+((r.data?.active_sessions??0)===1?"":"s")+' · '+(r.data?.events_24h??0)+' security events / 24h.'}
sync();sb.auth.onAuthStateChange(()=>sync());
}

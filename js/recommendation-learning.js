...
(function(){
  ...
  const api={
    ...
    async load(){
      ...
      const cat=new Map(), creator=new Map(), signals=new Map();
      ...
      this.profile.creators=[...creator.entries()].sort((a,b)=>b[1]-a[1]).slice(0,6).map(([id,score])=>({id,score:Math.round(score*10)/10}));

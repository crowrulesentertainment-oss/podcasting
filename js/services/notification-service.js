...
async function list(limit=50){const u=await window.CrowRulesData.guard("member"),db=await api();return db.from("podcast_notifications").select("*").eq("user_id",u.id).order("created_at",{ascending:false}).limit(limit)}

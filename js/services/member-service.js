/* CrowRules Podcasting — Member Action Domain Service 4.0 */
...
const RPC={followPodcast:"follow_my_podcast",unfollowPodcast:"unfollow_my_podcast",saveItem:"save_my_podcast_item",removeItem:"remove_my_podcast_item",library:"get_my_podcast_library",following:"get_my_podcast_following"};
...
async function followMember(id){const u=await member(),db=await api(),r=await db.from("podcast_member_follows").insert({follower_user_id:u.id,followed_user_id:id});if(r.error)throw r.error;return r.data}
async function unfollowMember(id){const u=await member(),db=await api(),r=await db.from("podcast_member_follows").delete().eq("follower_user_id",u.id).eq("followed_user_id",id);if(r.error)throw r.error;return r.data}
async function markNotificationRead(id){const u=await member(),db=await api(),r=await db.from("podcast_notifications").update({is_read:true}).eq("id",id).eq("user_id",u.id);if(r.error)throw r.error;return r.data}

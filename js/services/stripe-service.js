...
async function status(){const u=await window.CrowRulesData.requireAuth(),db=await window.CrowRulesData.getClientAsync();return db.from("cr_podcast_stripe_accounts").select("*").eq("user_id",u.id).maybeSingle()}
...
window.CrowRulesStripe={version:"1.0",status,begin,refresh};
})();
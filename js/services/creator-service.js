...
async function creator(){const u=await window.CrowRulesData.requireAuth(),db=await api();return db.from("creator_profiles").select("*").eq("user_id",u.id).maybeSingle()}
...
window.CrowRulesCreator={version:"1.0",creator,podcasts,episodes,getEpisode,savePodcast,saveEpisode};
})();
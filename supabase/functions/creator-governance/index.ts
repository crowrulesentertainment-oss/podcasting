...
export default {
  fetch: withSupabase({ auth: "user" }, async (req, ctx) => {
    ...
    if (action === "task-sync") {
      ...
      const rows=tasks.slice(0,500).map((t:any)=>({collection_id:String(t.collection_id||t.collectionId||""),task_key:String(t.task_key||t.key||""),task_name:t.task_name||t.name||null,owner_user_id:([t.owner_user_id,t.ownerUserId].find((v:any)=>typeof v==="string"&&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(v))||null),status:t.status==="COMPLETE"?"COMPLETE":t.status==="BLOCKED"?"BLOCKED":t.status==="IN_PROGRESS"?"IN_PROGRESS":"OPEN",target_date:(t.target_date||t.targetDate)&&!Number.isNaN(Date.parse(t.target_date||t.targetDate))?new Date(t.target_date||t.targetDate).toISOString():null,started_at:t.started_at||null,completed_at:t.completed_at||null,last_activity_at:t.last_activity_at||null,dependency_key:t.dependency_key||t.dependency||null}));
